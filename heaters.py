...
class Heater:
    def __init__(self, config, sensor):
        ...
        self.initmax_power = config.getfloat('max_power', 1., above=0., maxval=1.)
        self.max_power = self.initmax_power
    ...
    def temperature_callback(self, read_time, temp):
        with self.lock:
            ...
            self.control.temperature_update(read_time, temp, self.target_temp,self.max_power)
    ...
    def cmd_SET_HEATER_TEMPERATURE(self, gcmd):
        ...
        maxp = gcmd.get_float('POWER', self.initmax_power,above=0., maxval=1.)
        pheaters = self.printer.lookup_object('heaters')
        logging.info("power: %.3f", maxp)
        pheaters.set_temperature(self, temp)
        pheaters.set_max_power(self, maxp)
...
class ControlBangBang:
    def __init__(self, heater, config):
        ...
        self.heating = False
    def temperature_update(self, read_time, temp, target_temp, maxp):
        ...
        if self.heating and temp >= target_temp+self.max_delta:
            ...
            self.heating = True
        if self.heating:
            self.heater.set_pwm(read_time, maxp)
        else:
...
class ControlPID:
    ...
    def temperature_update(self, read_time, temp, target_temp,maxp):
        if self.Ki:
            self.temp_integ_max = maxp / self.Ki
...
class PrinterHeaters:
    ...
    def set_max_power(self,heater,maxpower):
        heater.set_maxp(maxpower)
